...
module.exports = {
  ...
  onMessage: {
    ...
    getLoginType: function(params, ewd) {
      var loginGlo = new ewd.mumps.GlobalNode(ewd.map.global.monitor, ['login']);
      if (loginGlo._exists) {
    ...
    checkForUpdates: function(params, ewd) {
      if (!ewdChild.checkForUpdates) {
        return {check: false};
      }

      var options = {
        url: 'http://ec2.mgateway.com/mgw/currentBuild',
        //url: 'http://localhost:8080/mgw/currentBuild',
        timeout: 5000
      };
      ...
      request(options, function (error, response, body) {
        if (error) {
          ewd.sendWebSocketMsg({
            ...
            message: {
              error: 'Error when contacting M/Gateway: ' + error
            },
            finished: true
          });
          return;
          ...
          try {
            ...
            ewd.sendWebSocketMsg({
              type: 'checkForUpdates',
              message: {
                error: 'Invalid JSON response from M/Gateway Web Site: ' + body
              },
              finished: true
            });
            return;
          }
          if (ewdChild.buildNo < respObj.buildNo) {
            ewd.sendWebSocketMsg({
              type: 'checkForUpdates',
              message: {
                update: respObj.buildNo
              },
              finished: true
            });
          }
    ...
    getUser: function(params, ewd) {
      ...
      var user = new ewd.mumps.GlobalNode(ewd.map.global.monitor, ['login', params.username])._getDocument();
      return {
        type: user.type,
    ...
    addUser: function(params, ewd) {
      ...
      var loginGlo = new ewd.mumps.GlobalNode(ewd.map.global.monitor, ['login', params.username]);
      if (loginGlo._exists) {
    ...
    confirmQRCode: function(params, ewd) {
      if (ewd.session.isAuthenticated) {
        ...
        var loginGlo = new ewd.mumps.GlobalNode(ewd.map.global.monitor, ['login', details.username]);
        loginGlo._delete();
    ...
    getSessionData: function(params, ewd) {
      if (ewd.session.isAuthenticated) {
        ...
        var session = new ewd.mumps.GlobalNode(ewd.map.global.session, ['session', params.sessid]);
    ...
    getSessions: function(params, ewd) {
      if (ewd.session.isAuthenticated) {
        ...
        var ewdSessions = new ewd.mumps.GlobalNode(ewd.map.global.session, ['session']);
        var data = [];
        var rowNo = -1;
    ...
    getWSUsers: function(params, ewd) {
      if (ewd.session.isAuthenticated) {
        var global = new ewd.mumps.GlobalNode(ewd.map.global.sessionNo,['EWDLiteServiceAccessId']);
    ...
    wsMgr_deleteUser: function(params, ewd) {
        var global = new ewd.mumps.GlobalNode(ewd.map.global.sessionNo,['EWDLiteServiceAccessId', params.target]);
        global._delete();
