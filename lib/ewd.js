...
var ewd = {
    ...
    thisProcess.on('message', function(response) {
      ...
      if (ewd.process[pid]) {
        ...
        if (ewd.childProcessMessageHandlers[response.type]) {
          ...
          if (!response.empty) ewd.log('No handler available for child process message type (' + response.type + ')', 3);
        }
        // release the child process back to the available pool
        //console.log('** response.type: ' + response.type);
        //console.log('** ewd.process for pid ' + pid + ': ' + ewd.process[pid].type);
        if (response.type !== 'EWD.exit' && response.release) {
          ewd.log('process ' + pid + ' returned to available pool; type=' + response.type, 3);
          delete ewd.process[pid].response;
          delete ewd.process[pid].type;
  ...
  handleAjaxRequest: function(content, response) {
    ...
    ewd.log("incoming Ajax request - data: " + JSON.stringify(params, null, 2), 1);
        ...
        fs.readFile(fileName, "binary", function(err, file) {  
          if (err) {
            ...
            fs.stat(fileName,function(err, stat) {
              ...
              if (fileName.indexOf(".xml") !== -1) contentType = "text/xml";
              var xdate = new Date();
  ...
  startWebServer: function() { 
    ...
    var webserverCallback = function(request, response) {
      ...
      request.once("end", function(){
        ...
        if (uri.substring(0,ewd.ajax.path.length)===ewd.ajax.path) {
          /*		    
  ...
  startExternalListener: function() {
    if (ewd.webSockets.externalListenerPort) {
      ...
      ewd.log('External Listener Port Disabled', 3);
    }
  },
