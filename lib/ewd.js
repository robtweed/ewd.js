...
var ewd = {
    ...
    webServiceRequest: function(responseObj) {
      ...
      if (response) {
        if (responseObj.error) {
          ...
          var header = {
            'Date': new Date().toUTCString(),
            'Content-Type': responseObj.contentType || 'application/json'
          };
  ...
  webSocketMessageHandlers: {
    ...
    webSocketMessage: function(messageObj, client) {
      ...
      if (messageObj.messageType === ewd.GraphQL.type) messageObj.service = ewd.GraphQL.module;
