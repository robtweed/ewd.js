    ...
    */
    ...
    webSocketMessage: function(messageObj) {
      ...
      if (ewdSession !== '') {
        ...
        var result;
				// create the EWD.js WebSocket domain
				var ewdWSD = domain.create(); 
				// this function traps all errors been throwed in asynchronous functions in user modules
				ewdWSD.on('error', function(err) {
					...
					try {
						...
						// so try to send the error nicely back to the master ...
						process.send({
							type: 'error',
        ...
        if (ewdChild.module[moduleName].onMessage && ewdChild.module[moduleName].onMessage[type]) {
					// you still need a try/catch block here to catch all synchronous errors, see https://gist.github.com/owenallenaz/7141699
          try {
						// ewdWSD.run has to return in this case, because the response is returned by the code inside ewdWSD.run
         ...
         // didn't include error handling here as this is deprecated stuff ...
    ...
    webServiceRequest: function(messageObj) {
      ...
      var result;
      if (ewdChild.database.type === 'mongodb') params.mongoDB = mongoDB;
						// create the EWD.js WebService domain
						var ewdWSD = domain.create();
						// this function traps all errors throwed in asynchronous functions
						// it can log to an error file 
						ewdWSD.on('error', function(err) {
							if (ewdChild.traceLevel >= 1) {
