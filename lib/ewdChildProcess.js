...
var ewdChild = {
  ...
  logToGlobal: function(message) {
    ...
    var gloRef = {global: ewdChild.logGlobal, subscripts: []};
    this.queueCommand('increment', gloRef, function(error, results) {
      var index = results._value;
      var gloRef = {global: ewdChild.logGlobal, subscripts: [index], value: logMessage};
...
process.on('message', function(messageObj) {
  ...
  if (ewdChild.messageHandlers[type]) {
    ...
    if (response) {
      // A Web Service handler can take responsibility for doing the process.send
      //  eg if it ends up inside a callback
      //  If so, the web services should return {exit: true} to force a return here:
      if (response.json && response.json.exit) return;
