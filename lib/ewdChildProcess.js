...
var ewdChild = {
  ...
  messageHandlers: {
    ...
    'EWD.pingProcess': function(messageObj) {
      return {
      ...
      if (ewdSession !== '') {
        ...
        if (ewdChild.module[moduleName].onMessage && ewdChild.module[moduleName].onMessage[type]) {
          ...
          try {
            ...
            if (ewdChild.traceLevel >= 1) ewdChild.log('Error attempting to execute ' + moduleName + ': ' + err, 1);
          ...
          if (ewdChild.module[moduleName].onSocketMessage) {
            ...
            if (ewdChild.traceLevel >= 2) ewdChild.log('Error: ' + moduleName + ' has no handler for message type ' + type, 2);
    ...
    webServiceRequest: function(messageObj) {
      ...
      if (ewdChild.webservice.authenticate) {
        ...
        if (ewdChild.traceLevel >= 3) ewdChild.log("hash: " + hash, 3);
        if (ewdChild.traceLevel >= 3) ewdChild.log("signature: " + signature, 3);
        if (hash !== signature) {
...
process.on('message', function(messageObj) {
  if (messageObj.type !== 'getMemory') {
    //ewdChild.timing = [{start: process.hrtime()}];
  ...
  //console.log('child process ' + process.pid + ' type = ' + type);  
  if (ewdChild.messageHandlers[type]) {
    //console.log('child process ' + process.pid + ' invoking handler at ' + process.hrtime(ewdChild.startns));
    //if (ewdChild.timing) ewdChild.timing.push({invokeHandler: process.hrtime(ewdChild.timing[0].start)});
    var response = ewdChild.messageHandlers[type](messageObj);
    //console.log('child process ' + process.pid + ' handler returned response at ' + process.hrtime(ewdChild.startns));
    //if (ewdChild.timing) ewdChild.timing.push({handlerDone: process.hrtime(ewdChild.timing[0].start)});
    if (response) {
      ...
      //console.log('child process ' + process.pid + ' sending response at ' + process.hrtime(ewdChild.startns));
      //if (ewdChild.timing) ewdChild.timing.push({sendingResponse: process.hrtime(ewdChild.timing[0].start)});
      //response.timing = ewdChild.timing;
      process.send(response);
      response = null;
      //console.log('child process ' + process.pid + ' done at ' + process.hrtime(ewdChild.startns));
      //if (ewdChild.timing) ewdChild.timing.push['childProcessDone'] = process.hrtime(if (ewdChild.timing) ewdChild.timing.push.start);
    }
    else {
      process.send({
        pid: process.pid,
