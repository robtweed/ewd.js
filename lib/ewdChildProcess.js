...
var EWD = {
  ...
  getSession: function(token) {
    ...
    if (ewdChild.previous.token && token === ewdChild.previous.token) {
      if ((now - ewdChild.previous.time) < 250) {
        return ewdChild.previous.session;
      }
    }
    //console.log('not using session cache');
    if (this.isTokenExpired(token)) {
      if (ewdChild.previous.token && ewdChild.previous.token === token) ewdChild.previous = {};
      return '';
    }
    else {
      ...
      ewdSession.setAuthenticated = function(authenticated) {
        ...
        if (authenticated) data = 1;
          // force a delay to ensure removal from previous cache in any other child process
          setTimeout(function() {
      ...
      ewdChild.previous.session = ewdSession;
      ewdChild.previous.token = token;
      ewdChild.previous.time = new Date().getTime();
      return ewdSession;
    }
  },
      ...
      fs.watch(path, function(event, filename) {
        ...
        if (event === 'change') {
          try {
            ...
            if (ewdChild.traceLevel >= 3) console.log(filename + " reloaded successfully");
          }
          catch (err) {
