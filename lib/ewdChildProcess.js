...
var EWD = {
  ...
  getSession: function(token) {
    ...
    if (this.isTokenExpired(token)) {
      ...
      ewdSession.sessid = database.get({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_sessid']}).data;
      ewdSession.app = database.get({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_appName']}).data;
      ewdSession.page = database.get({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_pageName']}).data;
      Object.defineProperty(ewdSession, 'isAuthenticated', {
        get: function() {
          return (+database.get({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_authenticated']}).data === 1);
        }
      });
      ...
      ewdSession.setAuthenticated = function(authenticated) {
        ...
        database.set({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_authenticated'], data: data});
  ...
  updateSessionExpiry: function(session) {
    ...
    if ((expiry - savedExpiry) > 0) {
      database.set({global: ewdChild.global.session, subscripts: ['session', sessid, 'ewd_sessionExpiry'], data: expiry});
    }
  }, 
...
process.on('message', function(messageObj) {
  ...
  */
  ...
  if (!ewdChild.messageHandlers[type] && ewdChild.Custom.messageHandlers[type]) {
