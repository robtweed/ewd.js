  ...
  messageHandlers: {
    ...
    initialise: function(messageObj) {
      ...
      if (ewdChild.database.type === 'cache') {
        ...
        database.open(ewdChild.database);
      }
      ...
      else if (ewdChild.database.type === 'mongo3') {
        mongo = require('mongo');
        mongoDB = new mongo.Mongo();
        database = new globals.Mongo();
        database.open(mongoDB, {address: ewdChild.database.address, port: ewdChild.database.port});
        ewdChild.database.namespace = '';
      }
